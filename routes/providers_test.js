    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            if (e.error.code == 11000 && e.error.keyPattern.email == 1) {
                resBody.msg = "This email is already associated with another provider. Please try with some different email."
            }
            else {
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        let { data, error } = await db.getData(ProviderModel, { email: email, password: pass, deactivate: false }, {
            _id: 0, id: "$_id", email: 1, providerName: 1, postcode: 1, address: 1, region: 1, postcode: 1, branch: 1, telephone: 1, mobile: 1, domain: 1, logoname: "$logo.url", deactivate: 1, emailVerified: 1, availability: 1, paypalMode: 1, cashMode: 1, flyer: "$flyer.url", category: 1
        })
...
router.post('/resetPassword', validation, async (req, res) => {
    ...
    try {
        ...
        let response = await db.getData(ProviderModel, { email: email, password: pass }, {
            _id: 0, id: "$_id", email: 1, userName: 1, name: 1, street: 1, number: 1, postcode: 1, place: 1, region: 1, mobile: 1, email: 1, deliveryNote: 1, picture: 1, deactivate: 1, emailVerified: 1
        })
        ...
        if (response.error === null) {
            if (response.data.length > 1) {
                ...
                let { data, error } = await db.updateOneData(ProviderModel, { _id: id }, { password: newpass }, {
                    _id: 0, id: "$_id"
                })
...
router.post('/sendOtp/', async (req, res) => {
    ...
    try {
        ...
        let { data, error } = await db.getData(ProviderModel, { email: email }, {
...
router.post('/resetOtpPassword', async (req, res) => {
    ...
    try {
        ...
        if (data.length > 1) {
            ...
            let response = await db.updateOneData(ProviderModel, { _id: id }, { password: newpass }, {
                _id: 1, id: "$_id", uid: 1, email: 1
            })
...
router.post('/updateUser', validation, async (req, res) => {
    ...
    upload(req, res, async (err) => {
        ...
        try {
            ...
            let { data, error } = await db.updateOneData(ProviderModel, { _id: newid }, { ...body, ...extraBody }, {
                _id: 0, id: "$_id"
            })
    ...
    try {
        ...
        if (error === null) {
            for (var i = 0; i < data.length; i++) {
                ...
                data1[i] = { providerName: data[i].providerName, address: data[i].address, number: data[i].number, postcode: data[i].postcode, place: data[i].place, region: data[i].region, branch: data[i].branch, telephone: data[i].telephone, mobile: data[i].mobile, email: data[i].email, emailVerified: data[i].emailVerified, domain: data[i].domain, openTime: data[i].openTime, closeTime: data[i].closeTime, orderStartDay: data[i].orderStartDay, orderEndDay: data[i].orderEndDay, paypalMode: data[i].paypalMode, cashMode: data[i].cashMode, paypalClientId: data[i].paypalClientId, paypalClientSecret: data[i].paypalClientSecret, deliveryCost: data[i].deliveryCost, minOrderCost: data[i].minOrderCost, deliveryCircle: data[i].deliveryCircle, deliveryApproxTime: data[i].deliveryApproxTime, description: data[i].description, Imprint: data[i].Imprint, logo: data[i].logo, availability: data[i].availability, deactivate: data[i].deactivate, iswelcome: data[i].iswelcome, community: data[i].community, files: { companyPresentation: v1, advertisement: v2, flyer: v3, jobAdvertisement: v4, menu: v5, info: v6, event: v7, advertisingVideo: v8 } }
            }
...
router.get('/getOneUserweb/:id', validation, async (req, res) => {
    ...
    try {
        ...
        let { data, error } = await db.getData(ProviderModel, { _id: id }, {
            _id: 0, id: "$_id", email: 1, providerName: 1, postcode: 1, address: 1, region: 1, postcode: 1, branch: 1, number: 1, telephone: 1, mobile: 1, domain: 1, logo: "$logo.url", deactivate: 1, emailVerified: 1, availability: 1, paypalMode: 1, cashMode: 1, flyer: "$flyer.url", category: 1, companyPresentation: "$companyPresentation.url", companyPresentationStartDay: 1, companyPresentationEndDay: 1, advertisement: "$advertisement.url", advertisementStartDay: 1, advertisementEndDay: 1, flyerStartDay: 1, flyerEndDay: 1, jobAdvertisement: "$jobAdvertisement.url", jobAdvertisementStartDay: 1, jobAdvertisementEndDay: 1, menu: "$menu.url", menuStartDay: 1, menuEndDay: 1, info: "$info.url", infoStartDay: 1, infoEndDay: 1, event: "$event.url", eventStartDay: 1, eventEndDay: 1, advertisingVideo: "$advertisingVideo.url", advertisingVideoStartDay: 1, advertisingVideoEndDay: 1, deliveryCost: 1, minOrderCost: 1, openTime: 1, closeTime: 1, orderStartDay: 1, orderEndDay: 1, paypalClientSecret: 1, paypalClientId: 1, deliveryCircle: 1, deliveryApproxTime: 1, iswelcome: 1, community: 1, description: 1, Imprint: 1, subject_line: 1, input_text: 1, post_start_date: 1, post_end_date: 1, picture_1: "$picture_1.url", picture_2: "$picture_2.url", feed_show_status: 1, company_presentation_subject_line: 1,
...
router.get('/getAllUsersWithLimit/', async (req, res) => {
    let resBody = {
        result: [],
        msg: "",
        status: false
    ...
    try {
        ...
        let { data, error } = await db.getData(ProviderModel, { iswelcome: true, deactivate: false }, {
            _id: 0, id: "$_id", email: 1, providerName: 1, postcode: 1, address: 1, region: 1, postcode: 1, branch: 1, telephone: 1, mobile: 1, domain: 1, logo: "$logo.url", deactivate: 1, emailVerified: 1, availability: 1, paypalMode: 1, cashMode: 1, flyer: "$flyer.url", category: 1, companyPresentation: "$companyPresentation.url", companyPresentationStartDay: 1, companyPresentationEndDay: 1, advertisement: "$advertisement.url", advertisementStartDay: 1, advertisementEndDay: 1, flyerStartDay: 1, flyerEndDay: 1, jobAdvertisement: "$jobAdvertisement.url", jobAdvertisementStartDay: 1, jobAdvertisementEndDay: 1, menu: "$menu.url", menuStartDay: 1, menuEndDay: 1, info: "$info.url", infoStartDay: 1, infoEndDay: 1, event: "$event.url", eventStartDay: 1, eventEndDay: 1, advertisingVideo: "$advertisingVideo.url", advertisingVideoStartDay: 1, advertisingVideoEndDay: 1, deliveryCost: 1, minOrderCost: 1, openTime: 1, closeTime: 1, orderStartDay: 1, orderEndDay: 1, paypalClientSecret: 1, paypalClientId: 1, deliveryApproxTime: 1, iswelcome: 1, community: 1, description: 1, Imprint: 1
        }, 4)
...
router.post('/getAllUsersWithLikeCountandWishlistCount/', validation, async (req, res) => {
    let resBody = {
        result: [],
        msg: "",
        status: false
...
router.get('/getTermsCondition/', async (req, res) => {
    ...
    var setting_doc = await Setting.findOne({ "title": 'terms and condition' }).exec();
    if (setting_doc) {
        resBody.status = true
...
router.put("/update-availability", async (req, res) => {
    ...
    try {
      // Loop through each availability object in the request body
      for (const { providerId, day, from, to, isActive } of availabilities) {
        // Validate each entry
        if (!providerId || !day || !from || !to) {
          resBody.msg = "All fields (providerId, day, from, to) are required for each availability object.";
          return res.status(400).json(resBody);
        ...
        if (isActive) {
          ...
          if (availability) {
            await ProviderAvailibility.deleteOne({ _id: availability._id });
          }
        }
